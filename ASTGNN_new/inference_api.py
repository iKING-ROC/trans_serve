...
@app.route('/TrafficFlowForcast', methods=['GET', 'POST'])
def hello_world():  # put application's code here
    # 入参两个字段:
    # location：地点，必须在['金湖东路-红莲街', '金源东路-红莲街', '兴贤路-乐民街', '金湖东路-乐民街', '金源东路-乐民街', '金源西路-乐民街',
    # '金湖东路-八于街', '金源东路-八于街', '金源西路-八于街', '金湖东路-乐安北一街', '金源东路-乐安北一街',
    # '兴贤路-乐安街', '金源东路-乐安街', '金源东路-双文北街', '金湖东路-乐安街']之中

    # timestamp：时间戳，格式必须为: xx:xx
    torch.npu.set_device(torch.device("npu:0")) 
    try:
        location = request.json['location']
        timeboard = request.json['timestamp']
        cut = int(timeboard.split(':')[0]) * 12 + int(timeboard.split(':')[1]) // 5
        #print("cut:"+cut)
        print("!!!!!!!!1")
        ...
        train_loader, train_target_tensor, val_loader, val_target_tensor, test_loader, test_target_tensor, _max, _min = load_graphdata_normY_channel1(
                ...
                num_of_days, num_of_weeks, DEVICE, batch_size, cut=cut)
        ...
        res = predict_main(100, test_loader, test_target_tensor, _max, _min, 'test')
        print(res)
        result = {
            "code": 200,
            "successful": "True",
...
if __name__ == '__main__':
    app.run(host='0.0.0.0', port=1125, debug=False)  # host必须写0.0.0.0
