...
def load_graphdata_normY_channel1(graph_signal_matrix_filename, num_of_hours, num_of_days, num_of_weeks, DEVICE,
                                  batch_size, shuffle=True, percent=1.0, cut=None):
    '''
    ...
    if cut is not None:
        cut = int(cut)
        test_x = test_x[cut:cut + batch_size, :, :]
        print('after cut text_x shape:', test_x.shape)
    test_x = test_x[:, :, 0:1, :]
    if cut is not None:
        cut = int(cut)
        test_target = file_data['test_target'][cut:cut + batch_size, :, :]
    else:
    ...
    # print('test_x_tensor:',test_x_tensor.shape)
    # sys.exit(0)
