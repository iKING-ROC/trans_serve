...
def read_and_generate_dataset_encoder_decoder(graph_signal_matrix_filename,
    ...
    for idx in range(data_seq.shape[0]):
        ...
        # sample：[(week_sample),(day_sample),(hour_sample),target,time_sample] = [(1,N,F,Tw),(1,N,F,Td),(1,N,F,Th),(1,N,Tpre),(1,1)]

    print(len(all_samples))
    dataset = [np.concatenate(i, axis=0)for i in zip(*all_samples)]
    # [(B,N,F,Tw),(B,N,F,Td),(B,N,F,Th),(B,N,Tpre),(B,1)]
    print(len(dataset))
    x = np.concatenate(dataset[:-2], axis=-1)
    # (B,N,F,T'), concat multiple time series segments (for week, day, hour) together

    target = dataset[-2]  # (B,N,T)
    # max-min normalization on x
    print(x.shape)
    (stats, train_x_norm, val_x_norm, test_x_norm) = MinMaxnormalization(x,x,x)
...
parser.add_argument("--config", default='configurations/PREDICT.conf', type=str,
